  ...
  <div class="dashboard-part">
    <div class="dashboard-part-top">
        <div class="dashboard-title-holder">
            <?php if($is_show_more): ?>
                    <div class="dashboard-part-top-title "><?php echo link_to('Địa điểm có cảm nhận mới','@showMoreComment?type=member_comment_pos') ?></div>
                    <div class="dashboard-part-top-link">
                        <?php echo link_to('Xem thêm', '@showMoreComment?type=member_comment_pos'); ?>
                    </div>
            <?php else: ?>
                <div class="dashboard-part-top-title">Địa điểm có cảm nhận mới</div>
            <?php endif;?>
        </div>
        <div class="clear"></div>
    </div>
    <div class="dashboard-part-content" id="pager_show_more_comment_pos_box">
        <div class="dashboard-pager">
           <?php if ($is_show_more): ?>
        <span class="current-page">Trang 1</span>
        <a class="dashboard-next" href="<?php echo url_for('@jqueryShowMoreComment?page=2&type=member_comment_pos') ?>" id="pager_show_more_member_comment_pos">&nbsp;</a>
          <?php endif ?>
        </div>
        <ul class="clearfix">
