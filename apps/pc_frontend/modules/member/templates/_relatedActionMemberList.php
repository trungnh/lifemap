...
<h3><?php echo link_to($sf_user->getMember()->getName(),'@member_profile?id='.$sf_user->getMember()->getId(),array('class'=>'underline'))?></h3>
      ...
      <?php $member = $action->getMember()?>
        <?php if($action->getActionName() == PluginPosLogTable::action_post_pos):?>
            <div class ="action-member-box">
                ...
                <div class="right-total-box">
                    <p class="action-member-title"><span class="member-name"><?php echo link_to($member->getName(),'@member_profile?id='.$member->getId())?></span> đã đăng địa điểm 
                      <?php if($pos) :?>
                      <?php echo link_to($pos->getTitle(),'@pos_profile?id='.$pos->getId(),array('class'=>'','target'=>'_blank'))?></p>
        ...
        <?php if($action->getActionName() == PluginPosLogTable::action_comment):?>
            <div class="action-member-box">
              ...
              ?>
                <div class="left-total-box">
                    <?php echo link_to(image_tag_sf_image($member->getImageFileName(),array('size'=>'40x40','class'=>'img_40x40')),'@member_profile?id='.$member->getId(),array('title'=>'Member'))?>
                </div>
                <div class="right-total-box">
                  <?php if($pos):?>
                  ...
                  <p class="action-comment"><?php if($posComment)echo $posComment->getContent();?></p>
                    <div class="location-item">
                        <?php echo link_to(image_tag_sf_image($pos->getImageFileName(),array('size'=>'70x70','class'=>'')),'@pos_profile?id='.$pos->getId(),array('title'=>$pos->getTitle()))?>
                        <div class="right-block">
                            <div class="location-title">
              ...
              ?>
                ...
                <div class="right-total-box">
                  ...
                  <p class="action-member-title"><span class="member-name"><?php echo link_to($member->getName(),'@member_profile?id='.$member->getId())?></span> đã đăng ảnh cho <?php echo link_to($pos->getTitle(),'@pos_profile?id='.$pos->getId(),array('class'=>'','target'=>'_blank'))?></p>
                    <?php if($posPhoto) echo image_tag_sf_image($posPhoto->getImageFileName(),array('size'=>'400x192','class'=>'','is_crop'=>0))?>
                    <div class="location-title">
                    <?php echo link_to($pos->getTitle(),'@pos_profile?id='.$pos->getId(),array('class'=>'underline','target'=>'_blank'));?>
                    </div>
                    <div class="location-address"><?php echo $pos->getAddress()?></div>
