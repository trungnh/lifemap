...
<?php else: ?>
<form class="<?php echo $options->getRaw('class'); ?>" action="<?php echo $options->getRaw('url') ?>" method="<?php echo $options['method'] ?>"<?php if (!empty($options['isMultipart'])): ?> enctype="multipart/form-data"<?php endif; ?>>
...
?>
  <tr>
    <td class="tb-composebox-label"><?php echo $field->renderLabel();?>:</td>
...
<?php include_slot('form_table') ?>
   ...
   <!--  </div> -->
<!-- </div> -->
<div class="lm-boxy-footer">
  <!--<ul class="lm-boxy-button lm-boxy-ok">
  <li>-->
  <?php foreach($forms as $form): echo $form->renderHiddenFields(); endforeach; ?>
  <input type="submit" class="lm-boxy-button lm-boxy-ok button-center" value="<?php echo $options['button'] ?>" />
  <!--</li>
  </ul> -->
  <?php
