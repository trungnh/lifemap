  ...
  */
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    if ($this->getOption('can_be_empty'))
    {
      ...
      $years  = array('' => $emptyValues['years']) + $years;
      if (!$dateTimeValue)
      {
    ...
    // days
    $widget = new sfWidgetFormSelect(array('choices' => $days), array_merge(array('class' => 'date_format'),$this->attributes, $attributes));
    $date['%day%'] = $widget->render($name.'[day]', $dayDefault);

    // months
    $widget = new sfWidgetFormSelect(array('choices' => $months), array_merge(array('class' => 'date_format'),$this->attributes, $attributes));
    $date['%month%'] = $widget->render($name.'[month]', $monthDefault);

    // years
    //$attributes['size'] = '5';
    $widget = new sfWidgetFormSelect(array('choices' => $years), array_merge(array('class' => 'date_format'), $this->attributes, $attributes));
