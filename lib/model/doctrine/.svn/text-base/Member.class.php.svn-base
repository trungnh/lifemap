...
class Member extends BaseMember implements opAccessControlRecordInterface
{
  ...
  public function isFriend($member_id)
  {
      $id = $this->getId();
    return true;
  }
  
  public function isFriendMe ( $member )
  {
    $relation = Doctrine::getTable('MemberRelationship')->retrieveByFromAndTo($this->getId(), $member->getId());
    if(!$relation)
    {
      $relation = Doctrine::getTable('MemberRelationship')->retrieveByFromAndTo($member->getId(), $this->getId());
    }
    if(!$relation)
    {
      return false;
  ...
  public function isFriendPreMe ( $member )
  {
    $relation = Doctrine::getTable('MemberRelationship')->retrieveByFromAndTo($this->getId(), $member->getId());
    if(!$relation)
    {
