...
class MemberTable extends opAccessControlDoctrineTable
{
  ...
  /*
   * lấy về danh sách các member mới nhất
   * @muoidv
   */

  public function getNewMemberList($num){
      if($num)
    ...
    /* pager danh sách thành viên mới */
    public function getNewMemberListPager($page = 1,$size = 10){
          $q = $this->createQuery()
            ->where('is_active =?',self::is_active)
            ->orderBy('created_at DESC');
    ...
    /* Tìm kiếm member theo text */
    public function getListMemberSearchByText($keyword,$num){
        $q = $this->createQuery()
          ->where('is_active =?',self::is_active)
    ...
    public function getListMemberSearchByTextPager($keyword, $page = 1, $size = 10){
        $q = $this->getListMemberSearchByTextQuery($keyword);

        return $this->getPager($q, $page, $size);
    }
    public function getListMemberSearchByTextQuery($keyword){
