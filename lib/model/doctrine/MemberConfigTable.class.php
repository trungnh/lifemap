...
class MemberConfigTable extends opAccessControlDoctrineTable
{
   ...
   * function checkLogin
   */
  public function checkLoginResult($pc_email,$password){
      if($this->retrieveByNameAndValue('pc_address', $pc_email))
        $memberAddress = $this->retrieveByNameAndValue('pc_address', $pc_email);
      else
        return null;
      if($this->retrieveByNameAndValue('password', $password))
        $memberPassword = $this->retrieveByNameAndValue('password', $password);
      else
        return null;
      if($memberAddress->getMember_id() == $memberPassword->getMember_id()){
