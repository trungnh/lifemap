...
public function searchMember($queryString,$memberId){
    $friendMemberIds = $this->getFriendMemberIds($memberId);
    ...
    $q = Doctrine::getTable('Member')->createQuery('m')
      ->whereIn('m.id', $friendMemberIds)     
      ->andwhere('m.name LIKE ?',$queryString.'%')      
      ->limit(10)
      ->execute();
     return $q;
  }
  ...
  public function getSomeFriendMemberIds($memberId,$num = null)
  {
    static $queryCacheHash;
    $result = array();
    if($num == null){
  ...
  public function appendRules(Zend_Acl $acl, $resource = null)
  {
   ...
   public function getIsFriendMember($memberId,$sendMemberId){
      $q = $this->createQuery('m')      
      ->where('m.member_id_to = ?',$sendMemberId)
      ->andWhere('m.member_id_from = ?',$memberId)
      ->andWhere('m.is_friend = ?', true)
      ->execute();
     return $q;
   }

   /* huent
    * Lay friend cua member
    */
   public function getFriendList($memberId,$num = null){
     $friendMemberIds = $this->getSomeFriendMemberIds($memberId,$num);
