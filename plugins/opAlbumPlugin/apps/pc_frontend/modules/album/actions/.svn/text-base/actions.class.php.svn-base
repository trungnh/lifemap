...
class albumActions extends opAlbumPluginActions
{
  ...
  public function executeListMember(sfWebRequest $request)
  {
    if($request->hasParameter('id') && $request->getParameter('id') != $this->getUser()->getMemberId()){
        $this->testIsFriend = true;
    }
    $this->pager = Doctrine::getTable('Album')->getMemberAlbumPager($this->member->getId(), $request->getParameter('page'),12, $this->getUser()->getMemberId());
  }
  ...
  public function executeShow(sfWebRequest $request)
  {
    ...
    $this->pager = Doctrine::getTable('AlbumImage')->getAlbumImagePager($this->album, $request->getParameter('page', 1), 9 );
