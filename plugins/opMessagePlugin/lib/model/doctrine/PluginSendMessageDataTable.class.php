...
class PluginSendMessageDataTable extends Doctrine_Table
{
  ...
  */
  ...
  public function deleteMessage($messageId)
  {
    ...
    ->delete()
    ->from('SendMessageData s')
    ->andWhere('s.id = ?',$messageId);
    return $deleted->execute();
  }
  /* count message sent or draft*/
   public function countSentOrDraftMessage($member_id,$is_send)
  {
    $q = $this->createQuery()
      ->where('member_id = ?', $member_id)
      ->andWhere('is_deleted = ?', false)
      ->andWhere('is_send = ?', $is_send);
    return $q->count();
  } 
}
