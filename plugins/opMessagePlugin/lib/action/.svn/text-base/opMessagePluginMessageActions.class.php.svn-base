 ...
 */
  ...
  public function executeList(sfWebRequest $request) {
    if($request->getParameter('isSend'))
      $this->isSend = $request->getParameter('isSend');
    ...
    switch ($this->messageType) {
      ...
      if($this->getUser)
        ...
        $this->pager = call_user_func(array($object, $function),
          $id,
          $request->getParameter('page', 1), /*!page ->$request = 1*/
  ...
  public function executeSendToFriend(sfWebRequest $request) {
    if ($params = $request->getParameter('message')) {     
      ...
      //Mutile sender       
        $sendMemberIds = $request->getParameter('send_member_ids');
        
        foreach($sendMemberIds as $sendMemberId) {               //        
          $this->message = Doctrine::getTable('SendMessageData')->find($params['id']);
          ...
          if ($request->isMethod(sfWebRequest::POST)) {
            ...
            $params['send_member_id'] = $sendMemberId; //set $params['send_member_id']
            unset ($params['send_member_ids']);//bo nhung params[]ngoai form
            $this->form->bind(
                $params,
         ...
         if ($this->message->getIsSend()) {                                   
                  $this->redirect("@sendList?isSend=". 1);
                }
              else {                                    
                  $this->redirect("@draftList?isSend=". 0);
              }   
        return sfView::INPUT;
      }
    }
    ...
    /* $sendMemberId: Id nguoi nhan*/
    /* $this->getUser()->getMemberId(): Id nguoi gui*/
    $this->forward404If($sendMemberId == $this->getUser()->getMemberId());
    $this->form = new SendMessageForm($this->message, array(
        'send_member_id' => $sendMemberId
    ));
