...
class sfImageStorageDefault
{
  ...
  static public function getUrlToImage($filename, $size, $format, $absolute = false, $is_crop = false)
  {
    ...
    if($is_crop){
        $filepath = 'img/'.$format.'/'.$sizeDirName.'/'.$filename.'_crop.'.$format;
    }else{
  ...
  {
    $instance = self::createInstance($className);
    if(strpos($filename,'crop')){
        $filename = str_replace('_crop', '', $filename);
    }
