...
class sfImageHandler
{
  ...
  public static function getAllowedSize()
  {
    ...
    self::$allowedSize = array_merge(
      array('40x40' ,'48x48', '60x60', '76x76', '120x120', '150x150','155x138', '160x160', '170x170', '180x180', '240x320','550x550', '600x600','150x200'),
  ...
  {
    $contents = $this->storage->getBinarys();

    if(strpos($this->options['filename'], 'crop')){
        $this->options['is_crop'] = 1;
    }else{
        $this->options['is_crop'] = 0;
    }
    $info = $this->generator->resize($contents, $this->storage->getFormat(),$this->options['is_crop']);
